...
- [x] Tap-through 7-Day-forecast screen — tapping "7-DAY OUTLOOK" pushes to a full day/night breakdown per day (separate condition/temperature/precip-chance for daytime vs. nighttime, not just the Dashboard's combined hi/lo), plus sunrise/sunset and moon phase for today; see [../src/daily_forecast_ui.cpp](../src/daily_forecast_ui.cpp). `DailyForecastPoint` (`include/weather.h`) grew nighttime condition/precip fields + raw sun/moon event strings, parsed from `forecastDays[].nighttimeForecast`/`sunEvents`/`moonEvents` (already fetched, just newly filtered/parsed — no new endpoint). No moon illumination % — the API doesn't return one, unlike the mockup's own decorative value.
- [x] Endurance/soak-test tooling — `[env:tab5-endurance]` (see [platformio-and-ci.md](platformio-and-ci.md#endurance-testing)) auto-cycles through every screen while logging free heap, per-task stack high-water marks, and LVGL memory-pool usage on a structured line every cycle; `tools/analyze_endurance_log.py` parses a live or saved serial capture into per-tag CSVs and time-series plots. Not run in CI — manual, hardware-in-the-loop only.
