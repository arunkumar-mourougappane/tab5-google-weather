...
// docs/rendering.md's Icons section. No tap-to-navigate into
// hourly-detail/daily-forecast (those screens don't exist yet), no
// Day/Night toggle (this kiosk always runs the dark palette, same as
// every other screen in this firmware).
//
...
enum class IconSize { Hero, DayRow };

// Small lookup table, not folded into weather_icon.h - that header stays
// Arduino/LVGL-free (see its own comment) so it's testable under
// `pio test -e native`; this mapping is purely "which generated
// lv_image_dsc_t goes with which bucket/size" and only makes sense next
// to the LVGL widgets that consume it.
...
const lv_image_dsc_t *iconForBucket(WeatherIconBucket bucket, IconSize size) {
  ...
  return size == IconSize::Hero ? kHero[index] : kDayRow[index];
}
